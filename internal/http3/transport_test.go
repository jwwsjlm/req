...
import (
	...
	"github.com/jwwsjlm/req/v3/internal/testcert"
...
func TestNewClientConnAndRawClientConn(t *testing.T) {
	...
	// Start QUIC listener
	listener, err := quic.ListenAddr("127.0.0.1:0", serverTLSConfig, &quic.Config{EnableDatagrams: true})
	if err != nil {
	...
	if !clientConn.enableDatagrams {
		t.Fatal("expected datagrams to be enabled")
	}
	if !clientConn.enableExtendedConnect {
		t.Fatal("expected Extended CONNECT to be enabled")
	}
	if clientConn.additionalSettings[0xffd277] != 1 {
		t.Fatal("expected additional settings to be copied")
	}
	if clientConn.maxResponseHeaderBytes != 262144 {
		t.Fatalf("unexpected max response header bytes: %d", clientConn.maxResponseHeaderBytes)
	}
	...
	if !rawConn.ClientConn.enableDatagrams {
		t.Fatal("expected raw conn datagrams to be enabled")
	}
	if !rawConn.ClientConn.enableExtendedConnect {
		t.Fatal("expected raw conn Extended CONNECT to be enabled")
	}
